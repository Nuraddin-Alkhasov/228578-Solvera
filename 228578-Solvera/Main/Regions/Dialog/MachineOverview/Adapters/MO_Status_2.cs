...
{
    [ExportAdapter("MO_Status_2")]
    public class MO_Status_2 : AdapterBase
    {
        ...
        private void CloseExecuted(object parameter)
        {
            ...
            Views.MO_Status_2 v = (Views.MO_Status_2)iRS.GetView("MO_Status_2");
        ...
        private void DeleteExecuted(object parameter)
        {
            if (MessageBoxView.Show("@Status.Text26", "@Status.Text27", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.from.Data.Delete", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.from.Data.Delete", false);
                    CloseExecuted(null);
        ...
        private void UpdateExecuted(object parameter)
        {
            if (MessageBoxView.Show("@Status.Text29", "@Status.Text28", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.from.Data.Update", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.from.Data.Update", false);
                    CloseExecuted(null);
        ...
        protected override void OnViewLoaded(object sender, ViewLoadedEventArg e)
        {
            ...
            {
                SPData = ApplicationService.ObjectStore.GetValue("MO_Status_2_KEY") as SP;
                ApplicationService.ObjectStore.Remove("MO_Status_2_KEY");
...
;                switch (SPData.Type)
                {
                    case "Box": Content = new MO_Status_Box2(); break;
        ...
        private void SetData() 
        {
           ...
           // {
                try 
                {
                    ...
                    uint OrderId = (uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.to.Data.Header.OrderId");
                    if (OrderId > 0)
                    {
                    ...
                    uint BoxId = (uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.to.Data.Header.BoxId");
                    if (BoxId > 0)
                    {
                    ...
                    MR.Header.Id = (int)(uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.to.Data.Header.MRId");
                    LayersSet = 0;
                    if (MR.Header.Id > 0)
                    {
                    ...
                    uint ChargeId = (uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.to.Data.Header.ChargeId");
                    if (ChargeId > 0)
                    {
                    ...
                    uint RunId = (uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.to.Data.Header.RunId");
                    if (RunId > 0)
                    {
                ...
                //short ActualLayer = (short)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.Status.to.Data.Charge.Layers.Actual");
                //    if (ActualLayer == 0)
                //    {
                //        PaintA = " - - - ";
                //        PaintN = Layers[0];
