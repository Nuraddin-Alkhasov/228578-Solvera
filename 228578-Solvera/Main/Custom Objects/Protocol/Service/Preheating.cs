...
namespace HMI.CO.Protocol
{
    class Preheating
    {
        public Preheating(string _Start, string _End, string _Error)
        {
            ...
            TrendArchive = trendService.GetArchive("PZ");
        ...
        public IVariable MC_Run_Id { set; get; }
        public List<IVariable> PZ_Run_Id { set; get; } = new List<IVariable>();
        public int StartError { set; get; }
        public int EndError { set; get; }

        public IVariable SetPZTemp { set; get; }
        #endregion
        ...
        private void Start_Event(object sender, VariableEventArgs e)
        {
            if (e.Quality.Data == DataQuality.Good && e.Value != e.PreviousValue && (bool)e.Value)
            {
                ...
                Task.Run(() =>
                {
                    Run r = GetRun((int)(uint)MC_Run_Id.Value);
                    if (r.Id > 0)
                    {
                        WriteDateTimeToRun("PZ_S", r);
        ...
        private void End_Event(object sender, VariableEventArgs e)
        {
            if (e.Quality.Data == DataQuality.Good && e.Value != e.PreviousValue && (bool)e.Value)
            {
                End.Value = false;
                    ...
                    Run r = GetRun((int)(uint)PZ_Run_Id[0].Value);
                    if (r.Id > 0)
                {
                        WriteDateTimeToRun("PZ_E", r);
        ...
        private void Error_Event(object sender, VariableEventArgs e)
        {
            if (e.Quality.Data == DataQuality.Good && e.Value != e.PreviousValue && (bool)e.Value)
            {
                ...
                Task.Run(() =>
                {
                    ...
                    for (int i = 0; i < PZ_Run_Id.Count - 2; i++) 
                    {
                        if ((int)(uint)PZ_Run_Id[i].Value > 0)
                        {
                            r = GetRun((int)(uint)PZ_Run_Id[i].Value);
                            if (r.Id > 0)
                                WriteErrorsToRun(r);
                        }
                    }
        ...
        private void WriteActualValuesToRun(object sender, GetTrendsDataCompletedEventArgs e)
        {
            ...
            Task.Run( () =>
            {
                ...
                double PZTemp = (double)Data[0].YValue;
        ...
        private void WriteSetValuesToRun(Run run)
        {
            new MSSQLEAdapter("Orders", "Update SetValues " +
                                  "SET PZTemp = " + Math.Round((float)SetPZTemp.Value, 0) + " " +
